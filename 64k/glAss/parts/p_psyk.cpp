...
void pp_generateScene( float frame ) {
	...
	glNewList( PSYK_LIST, GL_COMPILE );
		...
		glBegin( GL_TRIANGLES );
			...
			x = (float)cos( i*M_PI2/CYL_SEGMENTS );
			y = (float)sin( i*M_PI2/CYL_SEGMENTS );

			x2 = (float)cos( (i+1.0)*M_PI2/CYL_SEGMENTS );
			y2 = (float)sin( (i+1.0)*M_PI2/CYL_SEGMENTS );
