	...
	/*-
		{
			...
			for( d=0; d<2; d++ ) {
				u1 = 0.1f+0.4f*(float)cos(t+d*1);
				u2 = 0.9f+0.4f*(float)sin(t-d*2);
				glTexCoord2f( u1, 0 );	glVertex3f( -1, 0.5f+(d/80), 0 );
				glTexCoord2f( u2, 0 );	glVertex3f( 1, 0.5f+(d/80), 0 );
				glTexCoord2f( u2, 1 );	glVertex3f( 1, -0.5f-(d/80), 0 );
				glTexCoord2f( u1, 1 );	glVertex3f( -1, -0.5f-(d/80), 0 );
			};
...
/*
		...
		unsigned char hex;
		for( int i=0; i<20; i++ ) {
			hex = (unsigned char)(128 + 127*(float)sin(t*(1+i)));
			wsprintf( s, "%02x", hex );
			float x = (float)i / 10;

			float y = 0.5f;

		glColor3f( 0.7f*x, 0.34f*x, 0*x );
			glaDrawString( 1-x, -y, 0, 0.025f, 0.025f, 0.04f, s );
			glaDrawString( -1+x, y, 0, 0.025f, 0.025f, 0.04f, s );

		glColor3f( 0.1f*x, 0.04f*x, 0*x );
			glaDrawString( -1+x, -y, 0, 0.05f, 0.05f, 0.04f, s );
			glaDrawString( 1-x, y, 0, 0.05f, 0.05f, 0.04f, s );
		};
		glEnd();
...
/*		p_silu_klutt( t/5, 0.2f, 1.7f );
		p_silu_klutt( t/2, 0.2f, 1.4f );
		p_silu_klutt( t/2, 0.2f, 1.4f );
		p_silu_klutt( t/4, 0.2f, 0.9f );
		p_silu_klutt( t/3, 0.2f, 1.3f );*/
