...
void pl_draw( POBJECT *obj, MATRIX *mat, VECTOR *loc, float xc, float yc ) {
	...
	for( i=0; i<obj->nLines; i++ ) {
		...
		aa = pl->a;
		bb = pl->b;
		//gfx_drawline( xc+aa.x, yc+aa.y, xc+bb.x, yc+bb.y, 255 );
		vector_sub( &aa, loc );
		vector_sub( &bb, loc );
		//gfx_drawline( xc+aa.x, yc+aa.y, xc+bb.x, yc+bb.y, 255 );
		vector_mul( &aa, mat );
		...
		if( pl_proj( xc, yc, &aa ) && pl_proj( xc, yc, &bb ) ) {
			gfx_drawline( (int)aa.x, (int)aa.y, (int)bb.x, (int)bb.y, 254 );
			gfx_drawline( (int)aa.x+1, (int)aa.y, (int)bb.x+1, (int)bb.y, 254 );
			gfx_drawline( (int)aa.x, (int)aa.y+1, (int)bb.x, (int)bb.y+1, 254 );
			gfx_drawline( (int)aa.x+1, (int)aa.y+1, (int)bb.x+1, (int)bb.y+1, 254 );
		};
	};
};
