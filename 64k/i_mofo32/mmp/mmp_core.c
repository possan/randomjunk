...
int mmp_loadsample( int sample_index, short *sample_data, long sample_samples ) {
	...
	sample->data = (short *)malloc( sample_samples*2 );
...
void mmp_convertpattern(uchar *srcPatt, MMPPATTERN *destPatt)
{
	...
	destPatt->nRows = 64;
	destPatt->pNote = (MMPNOTE*)malloc( sizeof(MMPNOTE)*mmp_module.nTracks*64 );
	dest = destPatt->pNote;
...
int mmp_loadmodule( unsigned char *module ) {
	...
	if( mmp_memcmp( mh.sign, "M.K.", 4 )==0 ) 
		mmp_module.nTracks = 4;
	if( mmp_memcmp( mh.sign, "M!K!", 4 )==0 ) 
		mmp_module.nTracks = 4;
	if( mmp_memcmp( mh.sign, "FLT4", 4 )==0 ) 
		mmp_module.nTracks = 4;
	if( mmp_memcmp( mh.sign, "OCTA", 4 )==0 ) 
	...
	mmp_module.pTrack = (MMPTRACK*)malloc( sizeof(MMPTRACK)*mmp_module.nTracks );
	...
	mmp_module.pPattern = (MMPPATTERN*)malloc( sizeof(MMPPATTERN)*mmp_module.nPatterns );
	for( i=0; i<mmp_module.nPatterns; i++ ) {
		unsigned char *patdata = (unsigned char *)malloc( 256*mmp_module.nTracks );
	...
	mmp_module.pSample = (MMPSAMPLE*)malloc( sizeof(MMPSAMPLE)*mmp_module.nSamples );
	...
	for( i=0; i<31; i++ ) {
		...
		if( modi->slength ) {
			...
			s->data = (short *)malloc( s->length*2 );
...
#endif
	mmp_module.pOrder = (long*)malloc( sizeof(long)*mmp_module.nLength);
