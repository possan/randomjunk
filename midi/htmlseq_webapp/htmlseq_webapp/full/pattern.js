...
/*
function blockMouseUp( block, el, evt )
{
	console.log( 'blockMouseUp', block, el, evt );
	Event.stopObserving( el, 'mouseup', blockMouseUp.bindAsEventListener( this, block, el ) );
	Event.stop( evt );
}
*/
	...
	if( mousemode == 'keyroll' )
	{
		if( nk != lastkeydown )
		{
			if( lastkeydown != -1 )
			{
				...
				notify( 'keyroll', { note:nk, keystate:'up' } );
			}
			
			console.log( 'keyroll, note on: '+nk );
			notify( 'keyroll', { note:nk, keystate:'down' } );
			lastkeydown = nk;
		}
		Event.stop( evt );
	}
...
//	console.log( 'noteMouseUp: '+px+', '+py );
	...
	if( mousemode == 'keyroll' )
	{
		if( lastkeydown != -1 )
		{
			...
			{
				console.log( 'keyroll, note off: '+lastkeydown );
				notify( 'keyroll', { note:lastkeydown, keystate:'up' } );
			}
			lastkeydown = -1;
		}
	}
	else if( mousemode == 'addnote' )
	{
		...
		var not = { id:'dummy', from:starttime, to:t1, note:nk, velocity:100 };
		g_CurrentPattern.notes.push( not );
		
		notify( 'addnote', { id:'dummy', note:nk, from:starttime, to:t1, velocity:100 } );
	...
	else if( mousemode == 'dragnote' || mousemode == 'dragnotestart' || mousemode == 'dragnoteend' )
	{
		...
		// lastnoteblock.to = t1 + d;
		// lastnoteblock.note = nk;
		notify( 'movenote', { id:lastnoteblock.id, note:lastnoteblock.note, from:lastnoteblock.from, to:lastnoteblock.to, velocity:100 } );
...
function noteDblClick( evt )
{
	...
	if( hit_idx == -1 )
	{
		...
		var not = { id:'dummy', from:nt, to:nt2, note:nk, velocity:100 };

		g_CurrentPattern.notes.push( not );
		// console.log( 'notify note added' );
		
		notify( 'addnote', { id:not.id, note:not.note, from:not.from, to:not.to, velocity:not.velocity } );
		console.log( g_CurrentPattern.notes );
	}
	else
	{
		notify( 'deletenote', { id:hit_id } );
...
{
	var oldname = '';
	var p = prompt( 'What do you want to call your cloned pattern?', oldnamn );
	if( p )
	{
...
{
	var el1 = $('duration');
	g_Viewport.duration = parseInt( el1.value )*100;
	renderPattern();
	renderAutomation();
}
